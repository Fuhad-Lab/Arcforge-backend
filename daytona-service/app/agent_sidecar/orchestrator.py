...
AUTH PERSISTENCE LAW (injected system prompt): if the app needs authentication, it should NEVER use sqlite or local storage. Only a real database is allowed — ASK THE USER which database to use (request_connector for Supabase; other choices honored) — and cookies (httpOnly, set/read server-side) for session state.
