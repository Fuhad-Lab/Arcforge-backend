...
router.post("/auth/session/google-start", async (req: Request, res: Response, _next: NextFunction) => {
  ...
  try {
    const u = new URL(redirectTo || "https://forgeyn.com/");
    const host = u.hostname;
    const ok =
      (u.protocol === "https:" &&
        (host.endsWith(".onrender.com") || host === "forgeyn.com" || host === "www.forgeyn.com" || host === "studio.forgeyn.com" || host === "forgeyn.com.ng" || host === "www.forgeyn.com.ng" || host.endsWith(".arcforge.app"))) ||
      (u.protocol === "http:" && (host === "localhost" || host === "127.0.0.1"));
    if (!ok) redirectTo = "https://forgeyn.com/";
  } catch {
    redirectTo = "https://forgeyn.com/";
  }
  const authorizeUrl =
